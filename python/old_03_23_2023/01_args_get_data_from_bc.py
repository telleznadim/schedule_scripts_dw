...
def define_logger(file_name):
    ...
    handler = logging.FileHandler(
        f'C:/Users/eviadmin/Documents/Datawarehouse/schedule_scripts/From_BC/python/logs/{file_name}.log')
...
def getWebServicePaginationChatGPT(date_time, endpoint, company_bc, company_short, token):
    ...
    df.to_csv("C:/Users/eviadmin/Documents/Datawarehouse/python_scripts/DataLake/prod/files/from_bc/" + company_short + "_" + endpoint + "_" +
              date_time.strftime("%m%d%y") + '.csv.gz', index=False, compression="gzip")
    logger.debug(
        f"Data saved to file: C:/Users/eviadmin/Documents/Datawarehouse/python_scripts/DataLake/prod/files/from_bc/{company_short}_{endpoint}_{date_time.strftime('%m%d%y')}.csv.gz")
...
def getWebServicePagination(date_time, endpoint, company_bc, company_short, token):
    ...
    # logger.debug(("FINAL FINAL NO VA MAS")

    df = add_dw_columns(df, date_time, endpoint, company_short, "BC")
    logger.debug(df)
    # logger.debug((df["DW_Timestamp"])
    df.to_csv("C:/Users/eviadmin/Documents/Datawarehouse/python_scripts/DataLake/prod/files/from_bc/" + company_short + "_" +
